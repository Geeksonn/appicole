...
const BeerItem: React.FunctionComponent<Props> = (props) => {
    ...
    return (
        <View className='flex flex-row justify-around h-36 mx-3 my-8 bg-white rounded-xl drop-shadow-[0_0_30px_rgba(0,0,0,0.05)]'>
            <View className='w-1/3 -mt-4'>
                <Image source={imgUrl} contentFit='cover' transition={1000} style={{ flex: 1 }} />
            </View>
            <View className='flex flex-col w-2/3 py-2'>
                <View className='flex justify-between items-center pr-2'>
                    <Text className='font-title font-bold text-brass-green text-xl'>{name}</Text>
                </View>
                <Text className='text-brass-grey text-sm'>{`${type} - ${degree_integer},${degree_decimal} %`}</Text>
